...
package {
	...
	public class Scores extends MovieClip {
		//constants
		private const defaultE:uint = 44;//default earth score
		private const defaultW:uint = 40;
		private const defaultA:uint = 64;
		private const defaultF:uint = 68;
		...
		private function setScoresPassingLimit():void {
			...
			scoresPassingLimit[1] = 4;
			scoresPassingLimit[2] = 6;
			scoresPassingLimit[3] = 4;
		}
		...
		private function updateSharedObject():void {
			...
			par.addKongScore("Restarts",restarts);
			par.addKongScore("Jumps",jumps);
			//don't add scores if they aren't new
			if(scores[0]!=defaultE) {
				par.addKongScore("Earth Score",scores[0]);
			}
			if(scores[1]!=defaultW) {
				par.addKongScore("Water Score",scores[1]);
			}
			if(scores[2]!=defaultA) {
				par.addKongScore("Air Score",scores[2]);
			}
			if(scores[3]!=defaultF) {
				par.addKongScore("Fire Score",scores[3]);
			}
		}
	}
}